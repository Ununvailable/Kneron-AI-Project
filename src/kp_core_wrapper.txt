...
int kp_set_npu_timeout(kp_device_group_t devices, unsigned int seconds)
{
    _kp_devices_group_t *_devices_grp = (_kp_devices_group_t *)devices;

    if ((KP_DEVICE_KL730 == _devices_grp->product_id) ||
        (KP_DEVICE_KL830 == _devices_grp->product_id)) {
        kdp2_ipc_cmd_set_npu_timeout_t cmd;
        cmd.magic_type = KDP2_MAGIC_TYPE_COMMAND;
        cmd.total_size = sizeof(kdp2_ipc_cmd_set_npu_timeout_t);
        cmd.command_id = KDP2_COMMAND_SET_NPU_TIMEOUT;
        cmd.timeout = seconds;
    ...
    for (int i = 0; i < _devices_grp->num_device; i++)
    {
        ...
        if (do_rest)
        {
            ...
            while(fw_type != KP_KDP2_FW_LOADER_V2)  //wait for fw back to kp_loader stage
            {
                ...
                ret = kp_usb_connect_multiple_devices_v2(1, port_ids, devs, 100);
                if ((ret != KP_USB_RET_OK) && (re_connect_device_times < 6)) {
                    re_connect_device_times++;
                    continue;
                } else if (ret != KP_USB_RET_OK) {
                    return KP_ERROR_DEVICE_NOT_EXIST_10;
...
static int _kp_set_up_inference_queues(kp_device_group_t devices, uint32_t image_count, uint32_t image_size, uint32_t result_count, uint32_t result_size)
{
#define INIT_MAX_BUF_COUNT     8
#define APPEND_MAX_BUF_COUNT    65535
#define MAX_BUF_SIZE            8192 // 10-KB unit

    _kp_devices_group_t *_devices_grp = (_kp_devices_group_t *)devices;
    int timeout = _devices_grp->timeout;
    kp_product_id_t product_id = _devices_grp->product_id;
    uint16_t image_count_for_increase = 0;
    uint16_t result_count_for_increase = 0;

    if (image_count > INIT_MAX_BUF_COUNT) {
        image_count_for_increase = MIN(image_count - INIT_MAX_BUF_COUNT, APPEND_MAX_BUF_COUNT);
        image_count = INIT_MAX_BUF_COUNT;
    }
    ...
    if (result_count > INIT_MAX_BUF_COUNT) {
        result_count_for_increase = MIN(result_count - INIT_MAX_BUF_COUNT, APPEND_MAX_BUF_COUNT);
        result_count = INIT_MAX_BUF_COUNT;
    }
    ...
    kp_usb_control_t kctrl;
    kp_usb_control_t kctrl_for_increase;
    int ret = KP_SUCCESS;
    ...
    kctrl_for_increase.command = KDP2_CONTROL_FIFOQ_ADD_BUFFER;
    kctrl_for_increase.arg1 = image_count_for_increase;
    kctrl_for_increase.arg2 = result_count_for_increase;
    ...
    for (int i = 0; i < _devices_grp->num_device; i++)
    {
        ...
        if (((KP_DEVICE_KL630 == product_id) || (KP_DEVICE_KL730 == product_id) || (KP_DEVICE_KL830 == product_id)) &&
            ((0 != image_count_for_increase) || (0 != result_count_for_increase))) {
            ret = kp_usb_control(ll_dev, &kctrl_for_increase, timeout);

            if (ret != KP_USB_RET_OK) {
                dbg_print("[%s] add more buffer to inference queue failed return code = [%d]\n", __func__, ret);
            }
        }
    }
