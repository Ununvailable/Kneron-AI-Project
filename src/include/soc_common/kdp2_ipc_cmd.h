...
enum
{
    ...
    KDP2_CONTROL_FIFOQ_ADD_BUFFER = 0x86,           // add more fifoq buffer
};
...
enum kdp2_command_id
{
    ...
    KDP2_COMMAND_SET_IE_TIMEOUT = 0xA17,
    KDP2_COMMAND_SET_NPU_TIMEOUT = 0xA18,
...
typedef struct
{
    uint32_t magic_type; // should be 'KDP2_MAGIC_TYPE_COMMAND'
    uint32_t total_size; // size of this data struct
    uint32_t command_id; // should be 'KDP2_COMMAND_SET_IE_TIMEOUT'
    uint32_t timeout;
} __attribute__((aligned(4))) kdp2_ipc_cmd_set_ie_timeout_t;

typedef struct
{
    uint32_t magic_type; // should be 'KDP2_MAGIC_TYPE_COMMAND'
    uint32_t total_size; // size of this data struct
    uint32_t command_id; // should be 'KDP2_COMMAND_SET_NPU_TIMEOUT'
    uint32_t timeout;
} __attribute__((aligned(4))) kdp2_ipc_cmd_set_npu_timeout_t;
