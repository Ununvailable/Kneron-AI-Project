 ...
 */
int usb_transport_initialize();

/**
 * Finalize USB abstraction layer
 * (Release JNI refs, cleanup)
 */
int usb_transport_finalize();
 ...
 */
int usb_transport_bulk_out(usb_device_handle_t* handle, uint8_t endpoint, const void* data, int length, int timeout_ms);
