...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KL730 Demo Generic Image Inference NPU Timeout Setting Example.')
    ...
    """
    setting timeout of the NPU inference timeout (Please call after the load model function)
    """
    try:
        print('[Setting NPU Timeout]')
        kp.core.set_npu_timeout(device_group=device_group, seconds=3)
        print(' - Success')
    except kp.ApiKPException as exception:
        print('Error: setting timeout of the NPU inference timeout fail, port ID = \'{}\', error msg: [{}]'.format(usb_port_id, str(exception)))
        exit(0)
